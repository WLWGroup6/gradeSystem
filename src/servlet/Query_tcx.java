...
public class Query_tcx extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String sql = "SELECT"+
				...
				"	WHERE"+
				"	(0.7*FriRelShip.grade+0.3*FriRelShip.pgrade)<60 and"+
				"	t_no = '" +t_no+"'";
		...
		try {
			while(rs.next()){
				...
				map.put("r_8", rs.getString("r_8"));
				list.add(map);
			}
			session.setAttribute("list", list);
			rs.close();
			db.close();
			response.sendRedirect("chongxiu.jsp");
